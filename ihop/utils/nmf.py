""" Taken from https://github.com/seawander/nmf_imaging """
    ...
    if not oneByOne:
        ...
        if len(mask.shape) == 2:
            if path_save is None:
                ...
                print('\t\t loading from ' + path_save + '_comp.npy for components.')
                if not os.path.exists(path_save + '_comp.npy'):
                    print('\t\t ' + path_save + '_comp.npy does not exist, calculating from scratch.')
                    for i in range(n_components):
                        print("\t" + str(i+1) + " of " + str(n_components))
                        n = i + 1
                        if (i == 0):
                    ...
                    # Load with numpy
                    W_assign = np.load(path_save + '_comp.npy')
                    H_assign = np.load(path_save + '_coef.npy')
