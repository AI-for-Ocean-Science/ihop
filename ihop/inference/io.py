...
def pace_chains_filename(pace_file:str, edict:dict, error:int, test:bool=False,
                        out_path:str=None, priors:dict=None):
    """
    Generates the filename for a PACE chains file.

    Args:
        pace_file (str): The PACE IHOP file.
        edict (dict): The input dictionary containing dataset information.
        error (int, str, None): The error value.
            If None, the file is assumed to be noiseless.
                And the error value is set to 1.
    ...
    # Hard-code this root
    if root == 'L23_X4_Y0_nmfnmf_22_chl_Rrs_dense_512_512_512_256':
        root = 'NN_22_chl'
    else:
        raise ValueError("Not ready for this root")

    # Parse the PACE filename
    iOC = pace_file.find('OC_AOP')
    pace = pace_file[:iOC-1]
    ...
    if error is None:
        ...
        error = 98
    elif error == 'PACE_TRUNC':
        error = 97
    else: # abs_sig = float
        pass
    # 
    chain_file = f'fit{prefix}_Rs{int(error):02d}_{pace}_{root}.npz'
    if priors is not None:
        if 'use_log_ab' in priors and priors['use_log_ab']:
            chain_file = f'{chain_file[:-4]}_logab.npz'
    if test:
